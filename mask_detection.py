...
def video_init(is_2_write=False,save_path=None): #get input source
    writer = None
    ...
    if is_2_write is True: #save the output
...
def mask_detection(is_2_write=False,save_path=None):
    ...
    id2class = {0: 'MASKE VAR', 1: 'MASKE YOK'} #0: has mask, 1:no mask
    ...
    while (cap.isOpened()):
        ...
        if ret:
            ...
            for idx in keep_idxs:
                ...
                cv2.putText(img, "%s: %.2f" % (id2class[class_id], conf), (int(xmin), int(ymax) +15),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, color,2) # text for detected faces( have mask or not )


            cv2.putText(img, "MASKELI "+str(mask_count)+" KISI",
                        (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2) # number of people have mask
            cv2.putText(img, "MASKESIZ "+str(noMask_count)+" KISI", (300, 50),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2) # number of people have not mask
...
if __name__ == "__main__":
    save_path = r"out.avi"
    mask_detection(is_2_write=True, save_path=save_path)